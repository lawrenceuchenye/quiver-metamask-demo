...
const Send: React.FC<Props> = ({ type }) => {
  ...
  const [isBillBatched, setIsBillBatched] = useState<boolean>(false);
      ...
      >
        <div className="header">
          <h1 style={{ fontFamily: "Monoton", fontWeight: "400" }}>
            ...
            <m.i
              className="fa-solid fa-box b-btn"
              whileTap={{ scale: 1.2 }}
              style={{ opacity: !isBillBatched && "0.5" }}
              onClick={() => {
                if (!pricingData) {
                ...
                if (billBatch) {
                  if (billBatch.length > 4) {
                    toast.error("CLEAR OLD BATCH TO CONTINUE", {
                      position: "top-right",
                      autoClose: 3000,
                      hideProgressBar: false,
                      closeOnClick: false,
                ...
                setIsBillBatched(true);
                setTimeout(() => {
                  setIsBillBatched(false);
                }, 3000);
              }}
            ></m.i>
          </h1>
